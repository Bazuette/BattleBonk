package libayon.gui.altmanager;
...
import libayon.gui.altmanager.components.GuiAccountList;
import libayon.gui.altmanager.impl.GuiAddAlt;
import libayon.gui.altmanager.impl.GuiAltLogin;
...
public class GuiAltManager extends GuiScreen {
	...
	public void drawScreen(int p_drawScreen_1_, int p_drawScreen_2_, float p_drawScreen_3_) {
		...
		if (BattleBonk.instance.getAltService().getCurrentService().equals(AltService.EnumAltService.MOJANG)) {
			drawCenteredString(this.mc.fontRendererObj, Strings.translateColors(status), scaledResolution.getScaledWidth() / 2, 6, -3158065);
			drawCenteredString(this.mc.fontRendererObj, "Accounts: " + BattleBonk.instance.getAccountManager().getAccounts().size(), this.width / 2, 20, -1);
		} else
	...
	protected void actionPerformed(GuiButton button) throws IOException {
		...
		case 6:
			...
			break;
		case 7:
			if (BattleBonk.instance.getAccountManager().getAccounts().size() == 0)
				return;

			registry = BattleBonk.instance.getAccountManager().getAccounts();
			random = new Random();
			randomAlt = registry.get(random.nextInt(BattleBonk.instance.getAccountManager().getAccounts().size()));
		...
		case 5:
			...
			if (returnVal == 0) {
				...
				try {
					for (String line : Files.readAllLines(Paths.get(chooser.getSelectedFile().getPath()))) {
						...
						BattleBonk.instance.getAccountManager().getAccounts().add(account);
					}
			...
			break;
		case 8:
			if (BattleBonk.instance.getAccountManager().getLastAlt() == null)
				return;
			this.loginThread = new AccountLoginThread(BattleBonk.instance.getAccountManager().getLastAlt().getEmail(), BattleBonk.instance.getAccountManager().getLastAlt().getPassword());
			this.loginThread.start();
			break;
		case 9:
			if (BattleBonk.instance.getAccountManager().getAccounts().isEmpty())
				return;
			BattleBonk.instance.getAccountManager().getAccounts().clear();
			break;
		}
	}
