...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float partialTicks) {
		...
		BattleBonk.instance.getEventManager().call(new Render2DEvent(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight()));
	...
	protected void renderTooltip(ScaledResolution sr, float partialTicks) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			if (BattleBonk.instance.isDefaultHotbar()) {
				...
				float f = this.zLevel;
				this.zLevel = -90.0F;
				float hue = ((System.currentTimeMillis() % 6000 +i * 90) / 6000f + i * 90);
				int color = Color.HSBtoRGB(hue, 0.5f, 0.97f);
				RenderUtils.drawModalRect(i - 90, 0, 9 * 20, 2, color);

				int layers = 20;

				for (int layer = 1; layer <= layers; layer++) {
					RenderUtils.drawModalRect(i - 90 + entityplayer.inventory.currentItem * 20, layer, 20, 1, new Color(0,0, 0, 150 - (1 * layer)).getRGB());
				}
