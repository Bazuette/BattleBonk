...
import libayon.event.events.player.ServerConnectingEvent;
import libayon.event.events.player.ServerJoinEvent;
		...
		(new Thread("Server Connector #" + CONNECTION_ID.incrementAndGet()) {
			public void run() {
				...
				try {
					...
					ServerConnectingEvent event = ((ServerConnectingEvent) BattleBonk.instance.getEventManager().call(new ServerConnectingEvent(ip, port)));

					cancel = event.isCancelled();

					if (cancel) {
						GuiConnecting.logger.error("Connection to the server cancelled");
						GuiConnecting.this.mc.displayGuiScreen(new GuiDisconnected(GuiConnecting.this.previousGuiScreen, "connect.failed", new ChatComponentTranslation("disconnect.genericReason", event.getCancelReason()), false));
						return;
					}
					...
					new Thread(() -> {
						while (mc.currentScreen == null && mc.theWorld == null && mc.thePlayer == null && mc.getCurrentServerData() == null)
							synchronized (BattleBonk.instance.getEventManager()) {
								if (cancel)
									return;
							}
						...
						BattleBonk.instance.getEventManager().call(new ServerJoinEvent(mc.getCurrentServerData()));
					}).start();
