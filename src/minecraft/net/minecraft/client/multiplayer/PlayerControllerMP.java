...
import libayon.module.modules.combat.Reach;
...
public class PlayerControllerMP {
    ...
    public float getBlockReachDistance() {
        float reach = 0;
        if (BattleBonk.instance.getModuleManager().getModule(Reach.class).isToggled() && Reach.blockrange != 0.0f) {
            reach = Reach.blockrange;
        } else {
            if (this.currentGameType.isCreative()) {
                reach = 5.0f;
            } else {
