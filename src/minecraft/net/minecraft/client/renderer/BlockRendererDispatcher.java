	...
	public boolean renderBlock(IBlockState state, BlockPos pos, IBlockAccess blockAccess, WorldRenderer worldRendererIn) {
		try {
			...
			if (i == -1) {
				...
				switch (i) {
				case 1: {
					FluidRenderEvent fluidRenderEvent = (FluidRenderEvent) BattleBonk.instance.getEventManager().call(new FluidRenderEvent(pos, state));
				...
				case 2:
					return false;
				case 3: {
					BlockRenderEvent blockRenderEvent = (BlockRenderEvent) BattleBonk.instance.getEventManager().call(new BlockRenderEvent(state.getBlock()));
