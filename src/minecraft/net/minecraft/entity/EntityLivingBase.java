...
import libayon.event.events.player.LadderClimbEvent;
...
public abstract class EntityLivingBase extends Entity {
	...
	public float upwardsJumpMotion = 0;
	...
	protected void jump() {
		JumpEvent event = (JumpEvent) BattleBonk.instance.getEventManager().call(new JumpEvent(getJumpUpwardsMotion()));
		if (!event.isCancelled()) {
		this.motionY = event.jumpMotion;
			...
			if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
				if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer) this).capabilities.isFlying) {
					...
					if (this.isCollidedHorizontally && this.isOnLadder()) {
						LadderClimbEvent event = (LadderClimbEvent) BattleBonk.instance.getEventManager().call(new LadderClimbEvent(0.2D));
						if (!(event.isCancelled()))
